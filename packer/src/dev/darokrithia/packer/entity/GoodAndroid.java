...
public class GoodAndroid extends Mob{
	...
	public GoodAndroid(float x, float y, Handler handler) {
		...
		downAnimation = new Animation(Assets.alliedDownMove, 100);
		upAnimation = new Animation(Assets.alliedUpMove, 100);
		rightAnimation = new Animation(Assets.alliedRightMove, 100);
		leftAnimation = new Animation(Assets.alliedLeftMove, 100);
	...
	private Entity getClosest() {
		...
		for(int i = 0; i < handler.getEntityHandler().getEntities().size(); i++){
			if (handler.getEntityHandler().getEntities().get(i).getClass() == EvilAndroid.class){
				el.add(handler.getEntityHandler().getEntities().get(i));
			}
		}
		
		EvilAndroid e =  new EvilAndroid (1000000,10000000,handler);
		double d = Math.sqrt( ((e.getX() - this.getX())*(e.getX() - this.getX())) + ((e.getY() - this.getY())*(e.getY() - this.getY())));
	...
	public void render(Graphics g) {
		if(currentHealth >= 0){
			if (directionToClosest () == 2){
				...
				g.drawImage(Assets.alliedStatic, (int) centerX, (int) centerY, width, height, null);
			}
		}

		g.setColor(Color.red);
		g.fillRect((int) centerX, (int) (centerY -6), (int) (currentHealth/15.625), 3);
	}
